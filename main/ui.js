...
import { ENEMY_ATTRIBUTE_DEFENSE, ENEMY_ATTRIBUTE_FIRE_IMMUNITY, ENEMY_ATTRIBUTE_FLYING, ENEMY_ATTRIBUTE_ICON_MAP, ENEMY_ATTRIBUTE_MOBILE, ENEMY_ATTRIBUTE_REPEATABLE, getEnemyImage, getEnemyPosition, getEnemyStatusAnchor, getEnemyUiAnchor, getFryMageIndicatorImage } from "./enemy.js";
import {
  canJacobUseDefend,
...
export function loadUiAssets() {
  return Promise.all([
    ...
    waitForImage(getFryMageIndicatorImage())
  ]);
}
...
function drawPatrickProjectileEffect(context, effect) {
  ...
  } else if (effect.spriteSheet === "fry-mage-indicator") {
    context.drawImage(
      getFryMageIndicatorImage(),
      0,
      0,
      8,
      8,
...
function getInventoryContainsText(player) {
  ...
  const labels = inventory
    ...
    .map((slot) => {
      ...
      if (slot.itemId === "fires-item") {
        return `[resource]${t("action_fires_item")} x${Math.max(0, Math.floor(Number(slot.quantity) || 0))}[/resource]`;
      }
      if (slot.itemId === "ketchup-item") {
        return `[strength]${t("action_ketchup_item")} x${Math.max(0, Math.floor(Number(slot.quantity) || 0))}[/strength]`;
      }
