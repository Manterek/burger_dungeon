...
const KOSTYA_SHADOW_SUMMON_ANIMATION_DURATION_MS = PATRICK_REGULAR_FRAME_DURATION_MS * 3;
  ...
  approachPromise.then(() => {
    ...
    dodgePromise.then(async (dodgeResult) => {
      ...
      if (defendingUnit === state.player && state.player.characterId === KOSTYA_CHARACTER_ID) {
        ...
        const defendedWithoutDamage = defendedDamage > 0 && damageTaken === 0;
        if (defendedDamage > 0) {
...
function createJacobBallProjectileEffect(state, target, actionId, options = {}) {
  ...
  const startY = Math.floor((playerPosition.y + 2.56) * TILE_SIZE);
  const endX = Math.floor((enemyPosition.x + 1.08) * TILE_SIZE);
  const endY = Math.floor((enemyPosition.y + 2.42) * TILE_SIZE);
  const distancePx = Math.hypot(endX - startX, endY - startY);
  const durationMs = Math.max(
  ...
  return {
    ...
    trailWidth: 3,
    trailStartYOffset: 8,
    arcHeight: Math.max(16, Math.floor(distancePx * 0.12)),
