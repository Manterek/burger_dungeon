...
const FIRES_SP_RESTORE = 10;
const FIRES_SP_REGEN_POWER = 1;
const FIRES_SP_REGEN_TURNS = 2;
const KETCHUP_HEAL = 15;
const KETCHUP_STRENGTH_POWER = 4;
const KETCHUP_STRENGTH_TURNS = 3;
  ...
  approachPromise.then(() => {
    ...
    dodgePromise.then(async (dodgeResult) => {
      ...
      const counterDamage = dodgeResult.result === "counter"
        ...
        : 0;
      if (dodgeResult.result !== "perfect" && dodgeResult.result !== "counter") {
        (actingEnemy.attackStatusEffects ?? []).forEach((effect) => {
          applyStatusToUnit(
            defendingUnit,
            effect.statusKey,
            effect.power,
...
function startTimedDodgeCheck(state, render, attacker, target, options = {}) {
  return new Promise((resolve) => {
    ...
    startEnemyAttackAnimation(attacker, effect);
...
function getPlayerAllyUnits(state) {
...
async function performFiresItem(state, render) {
  state.player.sp = Math.min(state.player.maxSp, (state.player.sp ?? 0) + FIRES_SP_RESTORE);
  applyStatusToUnit(state.player, "spRegen", FIRES_SP_REGEN_POWER, FIRES_SP_REGEN_TURNS);
  state.message = t("battle_fires_item", {
    sp: FIRES_SP_RESTORE,
    power: formatRomanNumeral(FIRES_SP_REGEN_POWER),
    turns: FIRES_SP_REGEN_TURNS
  });
  render();
  return "continue";
}

async function performKetchupItem(state, render) {
  getPlayerAllyUnits(state).forEach((unit) => {
    applyStatusToUnit(unit, "strength", KETCHUP_STRENGTH_POWER, KETCHUP_STRENGTH_TURNS);
  });
  state.player.hp = Math.min(state.player.maxHp, state.player.hp + KETCHUP_HEAL);
  state.message = t("battle_ketchup_item", {
    heal: KETCHUP_HEAL,
    power: formatRomanNumeral(KETCHUP_STRENGTH_POWER),
    turns: KETCHUP_STRENGTH_TURNS
  });
  render();
  return "continue";
}
...
async function performInventoryItemAction(action, state, render) {
  ...
  if ([SALAD_ITEM_ID, CHERRY_ITEM_ID, BOMB_ITEM_ID, FIRES_ITEM_ID, KETCHUP_ITEM_ID].includes(action.itemId)) {
    const consumed = consumeInventoryItem(state.player, action.inventorySlotIndex, 1);
    if (consumed <= 0) {
      return "continue";
    }
    ...
    if (action.itemId === FIRES_ITEM_ID) {
      return performFiresItem(state, render);
    }
    if (action.itemId === KETCHUP_ITEM_ID) {
      return performKetchupItem(state, render);
    }
...
function awardPendingEnemyDrops(state) {
  ...
  state.enemies.forEach((enemy) => {
    ...
    (enemy.lootTable ?? []).forEach((drop) => {
      if (!drop?.itemId || !(Number(drop.chance) > 0)) {
        return;
      }
      if (Math.random() < Number(drop.chance)) {
        addInventoryItem(state.player, drop.itemId, 1);
      }
    });
  });
}
...
async function playEnemyTimedProjectileEffect(state, render, attacker, target) {
  ...
  await playLinearProjectileEffect(
    ...
    effect,
    effect.impactSfxKey
      ? [{ progress: 1, sfxKey: effect.impactSfxKey }]
      : [{ progress: 1, playImpactSfx: false }]
  );
}
...
function createEnemyTimedProjectileEffect(state, attacker, target) {
  ...
  if (attacker?.projectileStyle === "fry-mage") {
    return {
      ...
      trailColor: "#ff3f48",
      trailWidth: 4,
      rotationAngle: Math.atan2(targetAnchor.y - attackerAnchor.y, targetAnchor.x - attackerAnchor.x),
      spinTurns: 2.5,
      trailStartYOffset: 0,
      trailLength: 0.3,
      spriteSheet: "fry-mage-indicator",
      launchSfxKey: "throw",
      impactSfxKey: "impact"
    };
  }
...
function startEnemyAttackAnimation(attacker, effect = null) {
  if (!attacker) {
    return;
  }
  ...
  if (attacker.projectileStyle === "fry-mage") {
    ...
    playSfx(effect?.launchSfxKey ?? "throw");
  }
}
...
function settleTimedProjectileOnImpact(state, render, effect, onImpact = null) {
  ...
  window.setTimeout(() => {
    if (effect?.impactSfxKey) {
      playSfx(effect.impactSfxKey);
    }
    removeProjectileEffect(state, effect);
    onImpact?.();
    render();
