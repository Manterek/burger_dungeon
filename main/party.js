...
function getPatrickAnimatedFrameColumn(playerState) {
  ...
  return getLoopingPingPongFrameColumn(frameColumns, PATRICK_SLOWED_FRAME_DURATION_MS);
}
...
function getLoopingPingPongFrameColumn(frameColumns, frameDurationMs) {
  if (!Array.isArray(frameColumns) || frameColumns.length === 0) {
    return 0;
  }

  if (frameColumns.length === 1) {
    return frameColumns[0];
  }

  const cycleLength = frameColumns.length * 2 - 2;
  const elapsedFrames = Math.floor(performance.now() / Math.max(1, frameDurationMs));
