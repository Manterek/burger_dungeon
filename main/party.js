...
function getJacobSourceRect(playerState) {
  ...
  return getJacobAnimationFrameRect(
    JACOB_ANIMATION_ROW,
    frameColumns,
    getSequenceFrameColumn({ startedAt: 0 }, frameColumns, PATRICK_SLOWED_FRAME_DURATION_MS, { loop: true })
  );
}
...
function getJacobOverrideSourceRect(playerState) {
  ...
  if (override.type === "parry" && Number(override.endsAt ?? 0) > performance.now()) {
    return getJacobFrameRectWithLocalBounds(JACOB_PARRY_FRAME.row, JACOB_PARRY_FRAME.column);
  }
